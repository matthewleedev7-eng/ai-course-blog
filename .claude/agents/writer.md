...
**실측 vs 추정 구분 (필수)**: `keyFacts` 각 항목의 `basis` 필드를 정확히 채우세요.
- `basis: "measured"` (기본값) — 실제로 원본 파일을 세거나 대조해서 나온 값일 때만 사용 (예: "7일치 뉴스 35개" — 실제로 셌음)
- `basis: "estimate"` — 편집자의 판단/추측이 섞인 값일 때 (예: "다음 주 예상 이슈 수" 같은 전망성 수치)
본문 중 "다음 주 전망", "예상됩니다" 같은 추측성 문장 앞에는 **[편집자 코멘트]** 같은 명시적 표식을 붙여서, 실측 근거가 있는 트렌드 서술과 시각적으로 구분되게 하세요.
