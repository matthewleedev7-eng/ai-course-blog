...
매일 한국 시각 정오(12:00 PM)에 뉴스를 발행할 때 반드시 다음 6대 철칙을 엄수해야 합니다:
...
5. **동일 사건 중복 게재 금지 (No Duplicate Coverage)**:
   - 새 후보를 확정하기 전, `npm run check:duplicates`(`tools/check-duplicates.mjs`)를 돌려서 sourceUrl 완전 일치·헤드라인 유사도 기준으로 기계적으로 대조하거나, 최근 5~7일간 이미 발행된 `src/content/news/*.md`의 headline과 sourceUrl을 직접 확인해 **같은 사건(같은 회사·같은 계약/발표)을 이미 다뤘는지 대조**합니다. 원문 발행일이 다르거나 표현이 조금 바뀌었어도 핵심 사건이 같으면 재게재하지 않습니다.
...
6. **정량적 주장(배수·%, 벤치마크) 검증**: 기사에 "N배 빠름", "N% 개선" 같은 수치가 있으면 (a) 비교 기준선, (b) 측정 방법, (c) 측정 시점이 원문에 명시돼 있는지 확인합니다. 셋 중 하나라도 없으면 그 수치를 빼거나 "(제조사 발표 수치, 제3자 검증 없음)"으로 명시합니다. 보도자료 수치를 검증 없이 그대로 옮기지 않습니다.
...
**실측 vs 추정 구분**: `keyFacts`의 `basis` 필드는 실제로 세거나 대조한 값이면 `"measured"`(기본값), 편집자의 판단/전망이 섞인 값이면 `"estimate"`로 정확히 표기합니다. 본문의 전망성 문장에는 **[편집자 코멘트]** 표식을 붙여 실측 서술과 구분합니다.
